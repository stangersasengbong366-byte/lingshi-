...
test("学法直播不把学习指南和期末复习规划计入产品正课", () => {
  ...
  const parsed = parseCourseWorkbookSheets(workbook(["数学"], { 数学: rows }), "live", "高一", ["数学"]);
  assert.deepEqual(parsed.数学.map((row) => row.title), ["暑期第一讲"]);
  assert.deepEqual(parsed.数学.map((row) => row.no), [1]);
});
